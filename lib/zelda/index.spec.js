...
describe('Zelda', function() {
  ...
  describe('#setOptions', function() {
    ...
    it('should not recognize tokens in urls when options not setted', function() {
      const zelda = new Zelda();
      const linkifiedText = zelda.collectUrls('http://google.com?$long token$');
      this.expect(linkifiedText).to.eql(['http://google.com?$long']);
    });

    it('should recognize tokens in urls when options not setted', function() {
      const zelda = new Zelda();
      zelda.setOptions({ tokens: ['$long token$'] });
      const linkifiedText = zelda.collectUrls('http://google.com?$long token$');
      this.expect(linkifiedText).to.eql(['http://google.com?$long token$']);
    });
