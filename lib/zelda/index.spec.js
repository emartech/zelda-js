...
describe('Zelda', function() {
  ...
  describe('#collectLinks', function() {
    ...
    it('should return urls and offsets from text', function() {
      const zelda = zeldaFactory();
      const links = zelda.collectLinks('http://google.com, https://www.google.com');
      this.expect(links).to.eql([
        { url: 'http://google.com', offset: 0 },
        { url: 'https://www.google.com', offset: 'http://google.com, '.length }
      ]);
    });
    ...
    it('should return offsets correctly when there are multiple urls in text', function() {
      ...
      this.expect(links).to.eql([
        ...
        { url: 'http://google.com?$token #2$', offset: 'http://google.com?$token #1$ '.length },
        {
          url: 'http://google.com?$token #3$',
          offset: 'http://google.com?$token #1$ http://google.com?$token #2$ '.length
        }
      ]);
    });
    ...
    it('should return offsets correctly when there are several tokens in text', function() {
      ...
      this.expect(links).to.eql([{ url: 'http://google.com?$token #3$', offset: '$token #1$$token #2$ '.length }]);
    });
